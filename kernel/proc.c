...
static process_t* _proc_create_internal(bool restore_interrupts, proc_type_t kind,
                                        proc_entry_point_t kernel_entry,
                                        unsigned char* user_code, size_t user_size) {
    asm volatile("cli");
    if (!init_done) { if (restore_interrupts) asm volatile("sti"); return NULL; }

    // Find free slot
    process_t* proc = NULL;
    for (int i = 0; i < MAX_PROCESSES; i++)
        if (!process_table[i].used) { proc = &process_table[i]; break; }
    if (!proc) { if (restore_interrupts) asm volatile("sti"); return NULL; }

    proc->kernel_stack = kmalloc(KERNEL_STACK_SIZE);
    if (!proc->kernel_stack) { if (restore_interrupts) asm volatile("sti"); return NULL; }

    // Page directory
    if (kind == USER_PROC) {
        proc->page_directory = vmm_create_user_directory();
        if (!proc->page_directory) { kfree(proc->kernel_stack); if (restore_interrupts) asm volatile("sti"); return NULL; }
    } else {
    ...
    if (kind == USER_PROC && user_code && user_size > 0) {
        physical_addr_t code_phys = pmm_alloc_block();
        physical_addr_t stack_phys = pmm_alloc_block();
        if (!code_phys || !stack_phys) { kfree(proc->kernel_stack); if (restore_interrupts) asm volatile("sti"); return NULL; }

        virtual_addr_t code_virt = 0x01000000;
        virtual_addr_t stack_virt = 0x01080000;
        user_stack_top = stack_virt + PMM_BLOCK_SIZE;

        vmm_map_page_dir(proc->page_directory, code_virt, code_phys, PTE_PRESENT | PTE_USER | PTE_READ_WRITE);
        vmm_map_page_dir(proc->page_directory, stack_virt, stack_phys, PTE_PRESENT | PTE_USER | PTE_READ_WRITE);

        memcpy((void*)(KERNEL_VIRTUAL_BASE + code_phys), user_code, user_size);
    }
    ...
    if (kind == USER_PROC) {
        ...
        // Push IRET frame
        kstack_ptr -= 5 * sizeof(kuint32_t);
        kuint32_t* frame = (kuint32_t*)kstack_ptr;
        frame[4] = 0x01000000;   // EIP
        frame[3] = 0x1B;         // CS
        frame[2] = 0x202;        // EFLAGS
        frame[1] = user_stack_top; // ESP
        frame[0] = 0x23;         // SS

        proc->esp = (kuint32_t)kstack_ptr;
    } else {
        regs->ds = regs->es = regs->fs = regs->gs = 0x10;
        regs->cs = 0x08; regs->eip = (kuint32_t)kernel_entry; regs->eflags = 0x202;
    ...
    if (restore_interrupts) asm volatile("sti");
    return proc;
}
...
void proc_scheduler_run(registers_t *regs) {
    ...
    // Round-robin next process
    kuint32_t next_index = current_process_index;
    for (int i = 0; i < MAX_PROCESSES; i++) {
        next_index = (next_index + 1) % MAX_PROCESSES;
        if (process_table[next_index].used &&
            (process_table[next_index].current_state == RUNNING ||
             process_table[next_index].current_state == FIRST_RUN)) {
            break;
        }
    }

    if (next_index == current_process_index) return; // No other runnable process

    current_process_index = next_index;
    process_t* next = &process_table[next_index];

    // Update TSS for kernel stack
    kuint32_t kstack_top = (kuint32_t)next->kernel_stack + next->kernel_stack_size;
    tss_set_stack(0x10, kstack_top);

    // Switch page directory
    load_page_directory(next->page_directory);

    if (next->proc_type == USER_PROC && next->current_state == FIRST_RUN) {
        next->current_state = RUNNING;
        first_time_user_switch(next->esp); // Should iret to user process
    } else {
        context_switch(next->esp); // Normal kernel switch
    }
}
